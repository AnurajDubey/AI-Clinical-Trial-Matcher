  ...
  return (
    <div className="flex h-[36rem] flex-col overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-card">
      ...
      <div ref={scrollRef} className="scrollbar-slim flex-1 space-y-3 overflow-y-auto px-4 py-4">
        {messages.length === 0 && !liveEnabled && (
          <div className="space-y-2.5">
            <p className="text-xs leading-relaxed text-slate-500">
              This is the hosted demo. The navigator runs from{" "}
              <span className="font-semibold text-slate-700">recorded sessions</span> — pick one
              under <span className="font-semibold text-teal-600">&ldquo;Try a demo&rdquo;</span>{" "}
              below to watch the full agent loop: intake, live-trial reasoning, and bucketed
              results.
            </p>
            <p className="text-xs leading-relaxed text-slate-400">
              Want to chat with your own scenarios? Fork the repo and add an Anthropic API key to
              turn on live mode.
            </p>
          </div>
        )}
      ...
      {liveEnabled ? (
        ...
        <div className="border-t border-slate-100 bg-slate-50/60 p-3">
          <div className="flex items-start gap-2.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5">
            <span className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-lg bg-slate-100 text-slate-500">
              <svg
                viewBox="0 0 24 24"
                fill="none"
                stroke="currentColor"
            ...
            <p className="text-[11px] leading-relaxed text-slate-500">
              <span className="font-semibold text-slate-700">Live chat needs your own API key.</span>{" "}
              Fork the repo and set{" "}
              <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-[10px] text-slate-600">
                ANTHROPIC_API_KEY
              </code>{" "}
              to chat with the navigator directly.
            </p>
          </div>
        </div>
      )}
